...
class slider extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->model_name = 'slider_model';
		$this->item_big_photo_column_name = 'slider_big_photo';
		$this->item_thumb_photo_column_name = 'slider_thumb_photo';

		$this->change_item_photo_form_action = 'slider/changeItemPhoto';
	...
	{
		$slider_text_detail = $this->input->post('slider_text_detail');

		if ($slider_text_detail!='') 
		{
/*			$this->img_name = filterForeignChars($slider_text_detail);
...
*/
			...
			if ($image_up_and_resize == TRUE) 
			{
				...
				$insert_item_detail_to_db = $this->{$this->model_name}->insertNewItemDetail($slider_text_detail);
	...
	public function updateItemDetail()
	{
		...
		if ($slider_text_detail!='') 
		{
			$item_css = filterForeignChars($slider_text_detail);

			$update_item_detail = $this->{$this->model_name}->updateItemDetail($item_id ,$slider_text_detail);
